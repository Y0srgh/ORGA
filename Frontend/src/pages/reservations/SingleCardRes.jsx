...
const SingleCardRes = ({ model }) => {
    ...
    const footer = (<Button className='m-0' label="Voir les details" severity="secondary" icon="pi pi-list-check" text raised onClick={toggleDetails} />);
    ...
    return (
        <div className="card2 flex justify-content-center">
            <Card title={model[0].club} subTitle={model[0].motive} footer={footer} header={header} className="md:w-10rem m-0">
                    <div className='py-0'>
                    <p className="m-0">
                    date : {new Date(model[0].date).toLocaleDateString()}
                    </p>
                </div>
            </Card>
        </div>
